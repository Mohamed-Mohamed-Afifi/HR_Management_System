...
public class SearchSpec implements Specification<Project> {
    ...
    public Predicate toPredicate(Root<Project> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        Join<Project, Department> prjDept = root.join("department", JoinType.INNER);
        ...
        if (searchArgs.getDname() != null && !searchArgs.getDname().isEmpty()) {
            predicates.add(cb.like(prjDept.get("dname"), "%" + searchArgs.getDname() + "%"));
        }

        // Join with Employee
        Join<Project, Employee> prjEmp = root.join("employees", JoinType.INNER);
        if (searchArgs.getEmpName() != null && !searchArgs.getEmpName().isEmpty()) {
            predicates.add(cb.like(prjEmp.get("fname"), "%" + searchArgs.getEmpName() + "%"));
        }
        if(searchArgs.getProjectName()!=null &&!searchArgs.getProjectName().isEmpty()) {
        	predicates.add(cb.like(root.get("pname"), "%" +searchArgs.getProjectName() + "%"));
        }
